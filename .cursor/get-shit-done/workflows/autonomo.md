...
Inicialização via `init` de nível de marco:
...
**Se `roadmap_exists` for false:** Erro — "Nenhum ROADMAP.md encontrado. Execute `/gsd-novo-marco` primeiro."
**Se `state_exists` for false:** Erro — "Nenhum STATE.md encontrado. Execute `/gsd-novo-marco` primeiro."
...
| # | Fase | Status |
|---|------|--------|
| 5 | Estruturação de skills e descoberta de fases | Em progresso |
| 6 | Discussão inteligente | Não iniciada |
| 7 | Refinamentos em cadeia automática | Não iniciada |
| 8 | Orquestração do ciclo de vida | Não iniciada |
```
...
**Se has_context for true:** Pular discussão — contexto já coletado. Exibir:

```
Fase ${PHASE_NUM}: Contexto existe — pulando discussão.
```

Prosseguir para 3b.

**Se has_context for false:** Verificar se a discussão está desabilitada via configurações:
...
**Se SKIP_DISCUSS for `true`:** Pular a discussão por completo — a descrição da fase no ROADMAP é a especificação. Exibir:

```
Fase ${PHASE_NUM}: Discussão pulada (workflow.skip_discuss=true) — usando objetivo da fase do ROADMAP como especificação.
```

Escrever um CONTEXT.md mínimo para que o planejamento da fase (`planejar-fase`) downstream tenha entrada válida. Obter detalhes da fase:
...
**Modo:** Auto-gerado (discussão pulada via workflow.skip_discuss)
...
Todas as escolhas de implementação estão a critério do Claude — fase de discussão foi pulada por configuração do usuário. Utilize o objetivo da fase do ROADMAP, critérios de sucesso e convenções do codebase para guiar decisões.
...
Contexto do codebase será coletado durante a pesquisa do planejar-fase.
...
```bash
node "D:/projetos/Estudo/devsquad/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): contexto auto-gerado (discussão pulada)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
Após smart_discuss concluir, verificar que o contexto foi escrito:

```bash
PHASE_STATE=$(node "D:/projetos/Estudo/devsquad/.cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```

Verificar `has_context`. Se false → ir para handle_blocker: "Discussão inteligente para fase ${PHASE_NUM} não produziu CONTEXT.md."
...
Após executar-fase retornar, ler o resultado da verificação:
...
Ir para handle_blocker: "Execução da fase ${PHASE_NUM} não produziu resultados de verificação."
...
Exibir os itens, depois perguntar ao usuário via prompt conversacional:
...
Perguntar ao usuário via prompt conversacional:
...
Se ainda `gaps_found` após esta tentativa: Exibir "Lacunas persistem após tentativa de fechamento." e perguntar via prompt conversacional:
...
Executar discussão inteligente para a fase atual. Propõe respostas de áreas cinzentas em tabelas de lote — o usuário aceita ou substitui por área. Produz saída CONTEXT.md idêntica ao fluxo regular de discutir-fase (`discutir-fase.md`).

> **Nota:** Discussão inteligente é uma variante otimizada para autônomo da skill `gsd-discuss-phase` (comando `/gsd-discutir-fase`). Produz saída CONTEXT.md idêntica, mas usa propostas em tabelas de lote em vez de questionamento sequencial. O workflow `discutir-fase.md` permanece inalterado (por CTRL-03). Marcos futuros podem extrair isso para um arquivo de skill separado.
...
1. Palavras-chave do objetivo correspondem a termos de infraestrutura (em inglês ou português), por exemplo: "scaffolding", "plumbing", "setup", "configuration", "migration", "refactor", "rename", "restructure", "upgrade", "infrastructure", ou equivalentes como estruturação, tubulação (plumbing), configuração, migração, refatoração, renomear, reestruturar, atualização, infraestrutura
2. E os critérios de sucesso são todos técnicos (ex.: "file exists", "test passes", "config valid", "command runs", ou em português: arquivo existe, testes passam, config válida, comando roda)
3. E nenhum comportamento voltado ao usuário é descrito (sem formulações como "users can", "displays", "shows", "presents", ou "usuários podem", "exibe", "mostra", "apresenta")

**Se somente infraestrutura:** Pular Sub-passo 4. Ir diretamente para Sub-passo 5 com CONTEXT.md mínimo. Exibir:

```
Fase ${PHASE_NUM}: Fase de infraestrutura — pulando discussão, escrevendo contexto mínimo.
```

Usar estes padrões para o CONTEXT.md:
- `<domain>`: Limite da fase do objetivo do ROADMAP
- `<decisions>`: Uma única subseção "### À critério do Claude" — "Todas as escolhas de implementação estão a critério do Claude — fase de infraestrutura pura"
...
- Algo que usuários **CHAMAM** → interface: contratos, respostas, erros, autenticação
- Algo que usuários **EXECUTAM** → execução: invocação, saída, modos de comportamento, flags
...
Depois perguntar ao usuário via **prompt conversacional**:
- **header:** "Área {M}/{N}"
- **question:** "Aceitar estas respostas para {Nome da Área}?"
- **options:** Construir dinamicamente — sempre "Aceitar todas" primeiro, depois "Alterar Q1" até "Alterar QN" para cada pergunta (até 4), depois "Discutir mais fundo" por último. Máximo de 6 opções explícitas (prompt conversacional adiciona "Outro" automaticamente).

**Em "Aceitar todas":** Registrar todas as respostas recomendadas para esta área. Mover para a próxima área.

**Em "Alterar QN":** Usar prompt conversacional com as alternativas para aquela pergunta específica:
- **header:** "{Nome da Área}"
- **question:** "Q{N}: {texto da pergunta}"
- **options:** Listar as 1-2 alternativas mais "Você decide" (mapeia para a subseção À critério do Claude)
...
**Em "Discutir mais fundo":** Mudar para modo interativo para esta área apenas — perguntar uma pergunta por vez usando prompt conversacional com 2-3 opções concretas por pergunta mais "Você decide". Após 4 perguntas, perguntar:
- **header:** "{Nome da Área}"
...
Usar **exatamente** esta estrutura (idêntica à saída de `discutir-fase.md`):
...
```bash
node "D:/projetos/Estudo/devsquad/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): contexto discussão inteligente" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
Em **"Parar"**: Ir para handle_blocker com "Usuário parou — lacunas de auditoria permanecem. Execute /gsd-auditar-marco para revisar, depois /gsd-completar-marco quando pronto."
...
Mostrar o resumo, depois perguntar ao usuário via prompt conversacional:
...
Em **"Parar"**: Ir para handle_blocker com "Usuário parou — dívida técnica a tratar. Execute /gsd-auditar-marco para revisar detalhes."
...
Quando qualquer operação de fase falha ou um bloqueador é detectado, apresentar 3 opções via prompt conversacional:

**Prompt:** "Fase {N} ({Nome}) encontrou um problema: {descrição}"

**Opções:**
1. **"Corrigir e tentar novamente"** — Re-executar o passo que falhou (discussão, planejamento ou execução) para esta fase
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Retomar com: /gsd-autonomo --from {next_phase}
```
...
<success_criteria>
- [ ] Todas as fases incompletas executadas em ordem (discussão inteligente → planejar → executar cada)
- [ ] Discussão inteligente propõe respostas de áreas cinzentas em tabelas; o usuário aceita ou substitui por área
- [ ] Banners de progresso exibidos entre fases
- [ ] Executar-fase (`gsd-execute-phase`) invocado com --no-transition (autônomo gerencia transições)
...
- [ ] Falhas em planejar-fase (`gsd-plan-phase`) e executar-fase (`gsd-execute-phase`) roteiam para handle_blocker
...
- [ ] Discussão inteligente documenta relação com discutir-fase.md com nota CTRL-03
</success_criteria>
