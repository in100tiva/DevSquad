...
if [ -d "$TASKS_DIR" ]; then
  ...
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in D:/projetos/Estudo/devsquad/.cursor/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf D:/projetos/Estudo/devsquad/.cursor/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
